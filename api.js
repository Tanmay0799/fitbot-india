...
export async function askFitBot(messages, langName) {
  // Convert chat messages to the format Google Gemini expects
  const geminiMessages = messages.map(msg => ({
    role: msg.role === 'assistant' ? 'model' : 'user',
    parts: [{ text: msg.content }]
  }))

  // Add the FitBot persona rule as a system message at the absolute front
  ...
  // Safely extract text string from Google Gemini's response payload structure
  const full = data.candidates?.[0]?.content?.parts?.[0]?.text || 'Kuch issue hai. Dobara try karo!'
  
  // Parse any structural macros generated by the AI model for health summary logs
